...
function t1093__processGlobalObj() {
    ...
    if (typeof window.t1093_isMobile === 'undefined') {
        window.t1093_isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || ('ontouchend' in document && navigator.userAgent.indexOf('AppleWebKit') !== -1)
    }
...
function t1093__setPointerEventsToZeroBlock(rec) {
    ...
    var hasBG = abFilterBG !== 'none' || artBoardStyle.backgroundColor !== 'rgba(0, 0, 0, 0)' || artBoardStyle.backgroundImage !== 'none';
...
function t1093__processCurrentPopupOnShow(targetHook) {
    var currentPopup = document.querySelector('[data-tooltip-hook="' + targetHook + '"]');
    var currentPopupRec = currentPopup ? currentPopup.closest('.r') : null;
...
function t1093__setAnalyticsListeners(popup) {
    ...
    var currentHook = popup.getAttribute('data-tooltip-hook');
...
function t1093__closeOnLink(e) {
    ...
    if (!popupLink) return;
    var exceptionSelectors = ['.t978__tm-link', '.t966__tm-link', '.t794__tm-link', 'a[href="#closepopup"]', 'a[href="#closeallpopup"]', ];
...
function t1093__closePopupOnClick(e) {
    ...
    var currentPopup = e.target.closest('.t-popup');
    var currentPopupHook = currentPopup ? currentPopup.getAttribute('data-tooltip-hook') : '';
    var openPopupList = t1093__getPopupObjValue('openPopUpList');
...
function t1093__showCurrentPopup(popup) {
    ...
    var popupHook = popup.getAttribute('data-tooltip-hook');
...
function t1093__pauseAllVideo(popup) {
    if (!popup) return;
    var videoElems = Array.prototype.slice.call(popup.querySelectorAll('[data-elem-type="video"], [data-elem-type="gallery"]'));
    videoElems.forEach(function(element) {
        ...
        if (videoElem.tagName !== 'IFRAME' || !videoElem.src) return;
        if (videoElem.src.indexOf('&enablejsapi=1') !== -1) {
            videoElem.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*')
        }
        if (videoElem.src.indexOf('vimeo') !== -1 && videoElem.src.indexOf('&api=1') !== -1) {
